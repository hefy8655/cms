...
class GetDataModel extends RelationModel{
    ...
    public function getData($data){
        ...
        $timeArr['field_time'] = $data['field_time'];
        ...
        is_array($data['where']) && $where = $where+$data['where'];
        //搜索框
    ...
    private function dealTime($data){
        ...
        $field = isset($data['field_time'])?$data['field_time']:'create_time';
        $data[$field] = array('between', array($data['mintime'], $data['maxtime']));
        //清除没用的东西(防止条件报错)
        unset($data['mintime']);
        unset($data['maxtime']);
        unset($data['field_time']);
        return $data;
    }
    ...
    private function getStrip($data){
        ...
        for ($i=0; $i < count($data); $i++) {
            if(!empty(strstr($data[$i], '|'))){
                ...
                $cacheArray[$getExplode[0]] = (!empty($getExplode[0]) && !empty($getExplode[1])) ? M($getExplode[0])->db(1,DB_NAME)->getField($getExplode[1],true) : array();
            }else{
                continue;
            }
        }
